...
%Run only ONCE 
NumSet = 400; %330*12/10;
load('ImageStatsHuman_val_50_filtered.mat');
mkdir('expH_qr_classlabel');

for s = 1:NumSet
    
    [binL,cateL,imgL,typeL] = fcn_getPresentationInListExpH(AccumulateFiltered);
...
%     fclose(fileID);
    ...
    Mat = Mat(:);
    %Mat = Mat';
    keyframetype = ['expH_qr_classlabel/mturkSet_' num2str(s) '.txt'];
    fileID = fopen(keyframetype,'w');
    fprintf(fileID,'%d\n',Mat);
