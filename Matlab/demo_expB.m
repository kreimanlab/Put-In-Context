...
for imgnum = 1: NumImg %the current image id
    ...
    imgPortilaMaskfullname = [keyframetype '/trial_' num2str(node.bin) '_' num2str(node.classlabel)  '_' num2str(node.objIDinCate) '_PortillaMask.jpg' ];
    ...
    if exist(imgPortilaMaskfullname, 'file')~=2
        imgPortilaMaskfullname = [keyframetype '/' maskList(randperm(length(maskList),1)).name];
    end
...
%         %imshow(screen3T);
        ...
        fcn_WriteGIF(screen1, screen2, type2img, screen3, exptime(cfc,:), namegif,cf(cfc),keyframetype, repeat);
        counter = counter + 1;
    end
...
%         %imshow(PmaskT);
        ...
        fcn_WriteGIF(screen1, screen2, type2img, Pmask, exptime(cfc,:), namegif,cf(cfc),keyframetype, repeat);
        counter = counter + 1;
    end
...
%         %imshow(screen3T);
        ...
        fcn_WriteGIF(screen1, screen2, type8img, screen3, exptime(cfc,:), namegif,cf(cfc),keyframetype, repeat);
        counter = counter + 1;
    end
...
%         %imshow(PmaskT);
        ...
        fcn_WriteGIF(screen1, screen2, type8img, Pmask, exptime(cfc,:), namegif,cf(cfc),keyframetype, repeat);
        counter = counter + 1;
    end   
