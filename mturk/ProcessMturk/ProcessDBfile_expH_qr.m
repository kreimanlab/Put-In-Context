...
system('sqlite3 -header -csv db/expH_qr.db "select * from expH_qr;" > csv/results_expH_qr.csv');

fid = fopen('csv/results_expH_qr.csv');
out = textscan(fid,'%s');
strtotal = out{1,1};
pattern = 'http://kreiman.hms.harvard.edu/mturk/mengmi/keyframe';
...
fclose(fid);
save('Mat/mturk_expH_qr.mat','mturkData');