...
expname = 'expG'; #expA, expC, expD, expE, expH
...
trialtype = 4 #1, 2, 3, 4
if trialtype == 1:
    TSC = 1 #time step in each LSTM trial
elif trialtype == 2:
    TSC = 2
elif trialtype == 3:
    TSC = 4
...
def evaluate():
    ...
    # For each image
    for i, (imgs, binimgs, blurs, labels, imgname, imgobj, imgcontext, crimg) in enumerate(test_loader):
...
#        print(imgname[0][len(img_folder):-4])
        ...
        resultsavename = 'results_' + expname + '/'+ imgname[0][len(img_folder):-4] + '_' + str(trialtype) + '.mat' 
